node('maven') {
    ...
    stage ('App Push') {
        dir("source") {
            ...
            sh "cp app.py build-folder/app.py"
            sh "cp requirements.txt build-folder/requirements.txt"
            //sh "cp config-http-jmx.yaml build-folder/config-http-jmx.yaml"
            //sh "cp jmx_prometheus_javaagent-0.15.0.jar build-folder/jmx_prometheus_javaagent-0.15.0.jar"
